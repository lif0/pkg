...
func benchmarkEstimateSizeOf[T any](b *testing.B, getValue func() T) {
	...
	b.Run(TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(val)
			if res == reflectx.ErrFailEstimatePayload {
	...
	b.Run("*"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(ptrVal)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("[]"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(sliceValAny)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("*[]"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(ptrSliceVal)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("[]*"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(slicePrtValAny)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("*[]*"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(prtSlicePrtVal)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("[100]"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(arrayValAny)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("*[100]"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(ptrArrayVal)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("[100]*"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(arrayPrtVal)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("*[100]*"+TName, func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			res := reflectx.EstimatePayloadOf(prtArrayPrtVal)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
	...
	b.Run("[]"+TName+"{nil}", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			var arg []T
			res := reflectx.EstimatePayloadOf(arg)
			if res == reflectx.ErrFailEstimatePayload {
				fmt.Println(TName + " miss switch")
			}
		}
